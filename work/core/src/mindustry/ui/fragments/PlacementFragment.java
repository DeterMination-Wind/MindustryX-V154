...
public class PlacementFragment{
    ...
    public PlacementFragment(){
        ...
        Events.run(Trigger.update, () -> {
            ...
            if(mindustryX.VarsX.allUnlocked.changed("placement") || mindustryX.VarsX.itemSelectionWidth.changed() || mindustryX.VarsX.itemSelectionHeight.changed()){
                rebuild();
            }
        });
    }
    ...
    public void build(Group parent){
        int rowWidth = mindustryX.VarsX.itemSelectionWidth.get();
        int maxRow = mindustryX.VarsX.itemSelectionHeight.get();
        parent.fill(full -> {
            toggler = full;
            ...
            full.table(frame -> {
                    ...
                    top.add(new Table()).growX().update(topTable -> {
                        ...
                        if (Core.settings.getBool("hoveredTileInfo") && hoveredTile != null) {
                            topTable.row().table(t -> {
                                ...
                                t.add(hoverTile.floor().localizedName).ellipsis(true).wrap().growX();
                                if(hoverTile.block() != Blocks.air){
                                    t.add(" | ");
                                    t.image(hoverTile.block().uiIcon).size(iconSmall).padRight(4);
                                    t.add(hoverTile.block().localizedName).color(hoverTile.build == null ? Color.white : hoverTile.build.team.color).ellipsis(true).wrap().growX();
                                }
                                if(hoverTile.overlay() != Blocks.air){
                                    t.add(" | ");
                                    t.image(hoverTile.overlay().uiIcon).size(iconSmall).padRight(4);
                                    t.add(hoverTile.overlay().localizedName).ellipsis(true).wrap().growX();
                                }
                            }).growX().padTop(8).left();
                        }
                    });
                ...
                //commandTable: commanded units
                {
                    ...
                    commandTable.table(u -> {
                        ...
                        rebuildCommand = () -> {
                            ...
                            boolean arcExtra = mindustryX.VarsX.extendedCommandTable.get();
                                ...
                                if(arcExtra && units.size > 0){
                                    ...
                                    u.table(sp -> {
                                        ...
                                        if(assistCount > 0 && assistCount != units.size){
                                            sp.table(spp -> {
                                                arcSelectUnits(spp, "\uE86E", "进攻性单位", unit -> !unit.type.commands.contains(UnitCommand.assistCommand));
                                                arcSelectUnits(spp, "\uE86B", "辅助性单位", unit -> unit.type.commands.contains(UnitCommand.assistCommand));
                                            });
                                        }

                                        boolean hasFlyer = units.contains(Unitc::isFlying),
                                        ...
                                        if((hasFlyer ? 1 : 0) + (hasLand ? 1 : 0) + (hasNaval ? 1 : 0) >= 2){
                                            sp.table(spp -> {
                                                if(hasFlyer) arcSelectUnits(spp, UnitTypes.flare.emoji(), "飞行单位", Unitc::isFlying);
                    ...
                    blockCatTable.table(categories -> {
                        ...
                        if(maxRow > 5){
                            categories.button(Icon.zoom, Styles.clearTogglei, () -> {
                                ContentSelectDialog.once(content.blocks().select(block -> block.replaceable), control.input.block, block -> {
                ...
                frame.update(() -> {
                    ...
                    AdsorptionSystem.INSTANCE.getPlacementRect().reset(frame.x, frame.y, frame.getWidth(), frame.getHeight());
                });
            });
        });
    }
    ...
    void arcSelectUnits(Table table, String icon, String info, Boolf<Unit> cons){
        ...
        table.button(icon, Styles.cleart, () -> {}).tooltip(info).size(size).with(b->{
            ...
            b.clicked(KeyCode.mouseLeft, () -> {
                ...
                Events.fire(Trigger.unitCommandChange);
                UIExt.announce("RTS控制器：选择" + info + "！");
            });
            //right click -> remove
            b.clicked(KeyCode.mouseRight, () -> {
                control.input.selectedUnits.removeAll(cons);
                Events.fire(Trigger.unitCommandChange);
                UIExt.announce("RTS控制器：移除" + info + "！");
            });
