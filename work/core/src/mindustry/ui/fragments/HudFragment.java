...
public class HudFragment{
    ...
    public void build(Group parent){
        ...
        parent.fill(cont -> {
            ...
            wavesMain.table(s -> {
                if(mindustryX.VarsX.arcSpecificInfoTable.get()){
            ...
            cont.table(info -> {
                ...
                info.add(VarsX.version).color(Pal.accent).left();
                info.row();
    ...
    private Table makeStatusTableArc(){
        ...
        table.table(t -> {
            ...
            () -> Pal.health,
            () -> Math.min(player.dead() ? 0 : (player.unit().health / player.unit().maxHealth), 1))).height(18).growX();
            t.row();
    ...
    private boolean hideObjectives = false;
