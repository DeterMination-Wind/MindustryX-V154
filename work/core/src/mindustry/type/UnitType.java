    ...
    public void display(Unit unit, Table table){
        ...
        table.table(bars -> {
            ...
            if(payloadCapacity > 0 && unit instanceof Payloadc payload){
                ...
                StatUnit.blocksSquared.localized()), () -> Pal.items, () -> payload.payloadUsed() / payloadCapacity));
                bars.row();
        ...
        if(!unit.statuses().isEmpty()){
            table.row().table(t -> {
                for(StatusEntry entry : unit.statuses()){
                    ...
                    new Table(tt -> {
                        tt.left().bottom();
                        tt.label(() -> entry.effect.permanent || entry.time > Time.toHours * 10f ? "Inf" : UI.formatTime(entry.time)).style(Styles.outlineLabel);
                        tt.pack();
                    })).padLeft(8f);
                }
            });
            ...
            table.row().table(tt -> {
                ...
                tt.add("装甲");
                tt.row();
                tt.add(FormatDefault.format(unit.buildSpeedMultiplier));
                tt.add(FormatDefault.format(unit.dragMultiplier));
                tt.add(FormatDefault.format(unit.armorOverride >= 0.0F ? unit.armorOverride : unit.armor));
            }).growX().row();
        }
