...
public class EntityIO{
    ...
    void writeSync(MethodSpec.Builder method, boolean write, Seq<Svar> allFields) throws Exception{
        ...
        if(write){
            ...
            for(RevisionField field : revisions.peek().fields){
                ...
                boolean protocol155Field = isProtocol155SyncField(field);
                if(protocol155Field) cont("if(mindustryX.features.LogicExt.mockProtocol >= 155)");

                io(field.type, "this." + field.name, true);

                if(protocol155Field) econt();
            }
        }else{
    ...
    private boolean isProtocol155SyncField(RevisionField field){
        return name.equals("PlayerComp") && (field.name.equals("selectedBlock") || field.name.equals("selectedRotation"));
    }
